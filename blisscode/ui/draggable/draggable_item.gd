		...
		if event.button_index == MOUSE_BUTTON_LEFT and not event.pressed:
			if selected:
				...
				if areas_entered.size() > 0:
					...
					if slot_dropzone:
						if inventory.set_slot(slot_dropzone.get_slot_index(), item):
							areas_entered.clear()
							hide()
							return
...
func _on_area_entered(area: Area2D):
	if selected and area.is_in_group("dropzone"):
...
func set_item(i: Item):
	if sprite:
		...
		sprite.hframes = i.hframes
		sprite.vframes = i.vframes
		sprite.frame = i.frame
